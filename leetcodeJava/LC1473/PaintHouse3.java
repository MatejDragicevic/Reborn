class Solution
{
    ...
    public int minCost(int[] houses, int[][] cost, int m, int n, int target)
    {
        ...
        for (int i = 0; i <= target; i++) Arrays.fill(prevMemo[i], MAX_COST);
        ...
        for (int house = 1; house < m; house++)
        {
            ...
            for (int i = 0; i <= target; i++) Arrays.fill(memo[i], MAX_COST);
        ...
        for (int color = 1; color <= n; color++) minCost = Math.min(minCost, prevMemo[target][color - 1]);
